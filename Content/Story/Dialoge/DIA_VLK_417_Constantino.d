...
INSTANCE DIA_Constantino_BEER (C_INFO)
{
   npc          = VLK_417_Constantino;
   nr           = 1;
   condition    = DIA_Constantino_BEER_Condition;
   information  = DIA_Constantino_BEER_Info;
   permanent	= FALSE;
   description	= "Naprawdê nie mo¿esz nic zrobiæ?";
};

FUNC INT DIA_Constantino_BEER_Condition()
{
    if (Npc_KnowsInfo (other, DIA_Constantino_Heilung))
    {
    return TRUE;
    };
};
...
FUNC VOID DIA_Constantino_BEER_Info()
{
	...
	AI_Output (other, self ,"DIA_Constantino_BEER_15_06"); //Jasne.
...
instance DIA_Constantino_BEER_DRUNK (C_INFO)
{
   npc          = VLK_417_Constantino;
   nr           = 1;
   condition    = DIA_Constantino_BEER_DRUNK_Condition;
   information  = DIA_Constantino_BEER_DRUNK_Info;
   permanent	= FALSE;
   description	= "Wypi³em twoje piwo.";
};

func int DIA_Constantino_BEER_DRUNK_Condition()
{
    if (HeroKnowsInfoBeer == TRUE)
	&& (Npc_KnowsInfo (other, DIA_Constantino_BEER))
    {
    return TRUE;
    };
};
...
func void DIA_Constantino_BEER_DRUNK_Info()
{
    AI_Output (other, self ,"DIA_Constantino_BEER_DRUNK_15_01"); //Wypi³em twoje piwo.
    AI_Output (self, other ,"DIA_Constantino_BEER_DRUNK_10_02"); //I jak?
	...
	B_LogEntry (TOPIC_ConstantinoBeer,"Powiadomi³em Constantina o efektach eksperymentu. Nie by³ zbyt zachwycony");
    Log_SetTopicStatus (TOPIC_ConstantinoBeer, LOG_SUCCESS);
    B_GivePlayerXP (150);
    AI_Output (other, self ,"DIA_Constantino_BEER_DRUNK_15_08"); //Chocia¿ tyle za otarcie siê o œmieræ. 
	if (B_MenelBeerGiven == TRUE)
	{	
		AI_Output (other, self ,"DIA_Constantino_BEER_DRUNK_15_09"); //(do siebie) Znam kogoœ, komu móg³by podpasowaæ ten trunek.
		AI_Output (other, self ,"DIA_Constantino_BEER_DRUNK_15_10"); //Da³byœ mi jeszcze jeden kufel tego piwa?
		AI_Output (self, other ,"DIA_Constantino_BEER_DRUNK_10_11"); //A po co ci?
		AI_Output (other, self ,"DIA_Constantino_BEER_DRUNK_15_12"); //Mam kogoœ, komu bêdzie pasowa³ taki 'specjalny' napój.
		AI_Output (self, other ,"DIA_Constantino_BEER_DRUNK_10_13"); //Ech, no dobra, masz.
		AI_Output (self, other ,"DIA_Constantino_BEER_DRUNK_10_14"); //Tylko nikogo nie zabij, dobra?
		AI_Output (other, self ,"DIA_Constantino_BEER_DRUNK_15_15"); //Bêdê uwa¿a³.
		B_LogEntry (TOPIC_menel,"Zastanawiam siê, czy Menelaosowi podpasuje ten 'specjalny' trunek...");
		CreateInvItems (self, ItFo_MenelBeer, 1);
		B_GiveInvItems (self, other, ItFo_MenelBeer, 1);
	};
};
...
INSTANCE DIA_Constantino_BEER_AGAIN (C_INFO)
{
   npc          = VLK_417_Constantino;
   nr           = 1;
   condition    = DIA_Constantino_BEER_AGAIN_Condition;
   information  = DIA_Constantino_BEER_AGAIN_Info;
   permanent	= FALSE;
   description	= "Dasz mi jeszcze jedno piwo?";
};

FUNC INT DIA_Constantino_BEER_AGAIN_Condition()
{
    if (Npc_HasItems (other, ItFo_MenelBeer) == 0)
