...
INSTANCE DIA_Halvor_JAMES (C_INFO)
{
   npc          = VLK_469_Halvor;
   nr           = 1;
   condition    = DIA_Halvor_JAMES_Condition;
   information  = DIA_Halvor_JAMES_Info;
   permanent	= FALSE;
   description	= "Nie widzia³eœ mo¿e czegoœ co wygl¹da jak ma³a skrzyneczka?";
};

FUNC INT DIA_Halvor_JAMES_Condition()
{
    if (Npc_KnowsInfo (other, DIA_James_AGAIN))
    {
    return TRUE;
    };
};
...
FUNC VOID DIA_Halvor_JAMES_Info()
{
    AI_Output (other, self ,"DIA_Halvor_JAMES_15_01"); //Nie widzia³eœ mo¿e czegoœ co wygl¹da jak ma³a skrzyneczka?
    AI_Output (self, other ,"DIA_Halvor_JAMES_03_02"); //Mo¿e. A o co chodzi?
    AI_Output (other, self ,"DIA_Halvor_JAMES_15_03"); //Zgubi³ go taki jeden paladyn, James.
    AI_Output (self, other ,"DIA_Halvor_JAMES_03_04"); //James powiadasz?
    AI_Output (self, other ,"DIA_Halvor_JAMES_03_05"); //Faktycznie, mam skrzynkê podpisan¹ tym imieniem.
    AI_Output (self, other ,"DIA_Halvor_JAMES_03_06"); //Jednak nie mogê ci jej oddaæ za darmo.
    AI_Output (other, self ,"DIA_Halvor_JAMES_15_07"); //Co mam dla ciebie zrobiæ w zamian?
    AI_Output (self, other ,"DIA_Halvor_JAMES_03_08"); //Przynieœ mi trzy muszle rogowe, nieotwarte.
    AI_Output (self, other ,"DIA_Halvor_JAMES_03_09"); //Wtedy bêdê móg³ ci oddaæ t¹ skrzynkê.
    B_LogEntry(TOPIC_James_Eq,"Halvor chce 3 muszle rogowe w zamian za skrzynkê Jamesa.");
};
...
INSTANCE DIA_Halvor_JAMES_SOLVE (C_INFO)
{
   npc          = VLK_469_Halvor;
   nr           = 1;
   condition    = DIA_Halvor_JAMES_SOLVE_Condition;
   information  = DIA_Halvor_JAMES_SOLVE_Info;
   permanent	= FALSE;
   description	= "Mam muszle dla ciebie.";
};

FUNC INT DIA_Halvor_JAMES_SOLVE_Condition()
{
    if (Npc_KnowsInfo (other, DIA_Halvor_JAMES))
    && (Npc_HasItems (other, ItMi_Addon_Shell_02) >=3)
    {
    return TRUE;
    };
};
...
FUNC VOID DIA_Halvor_JAMES_SOLVE_Info()
{
	AI_Output (other, self ,"DIA_Halvor_JAMES_SOLVE_15_01"); //Mam twoje muszle.
    B_GiveInvItems (other, self, ItMi_Addon_Shell_02, 3);
    AI_Output (self, other ,"DIA_Halvor_JAMES_SOLVE_03_02"); //Wspaniale! Proszê, oto ta skrzynka,
    CreateInvItems (self, ItMi_JamesEq, 1);
    B_GiveInvItems (self, other, ItMi_JamesEq, 1);
    B_LogEntry (TOPIC_James_Eq,"Halvor da³ mi skrzynkê Jamesa. Teraz tylko muszê mu j¹ oddaæ.");
};
