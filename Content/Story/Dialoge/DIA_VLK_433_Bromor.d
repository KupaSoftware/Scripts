...
func void DIA_Addon_Bromor_LuciaGold_geben ()
{
	...
	B_GiveInvItems (other, self, ItMi_BromorsGeld_Addon,1);
	AI_Output			(self, other, "DIA_Addon_Bromor_LuciaGold_geben_07_01"); //Dziêkujê. IdŸ do Horace'a - on uda siê z tob¹ na piêterko.
	Bromor_Pay = 1; 
	MIS_Bromor_LuciaStoleGold = LOG_SUCCESS;
	Bromor_Hausverbot = FALSE;
	B_GivePlayerXP (XP_Addon_Bromor_LuciaGold);
...
func void DIA_Bromor_Pay_Info ()
{
	...
	if B_GiveInvItems (other, self, ItMi_Gold, 50)
	{
		...
		AI_Output (self, other, "DIA_Bromor_Pay_07_02"); //Zatem idŸ na górê z Horacem.
...
func void DIA_Bromor_DOPE_Info ()
{
	...
	AI_Output (self, other, "DIA_Bromor_DOPE_07_02"); //Je¿eli masz doœæ z³ota, mo¿esz iœæ na górê z Horacem.
};
...
INSTANCE DIA_Bromor_business (C_INFO)
{
   npc          = VLK_433_Bromor;
   nr           = 1;
   condition    = DIA_Bromor_business_Condition;
   information  = DIA_Bromor_business_Info;
   permanent	= FALSE;
   description	= "Mam pewn¹ sprawê.";
};

FUNC INT DIA_Bromor_business_Condition()
{
    if (MIS_Menel_Bitch == LOG_RUNNING)
    {
    return TRUE;
    };
};
...
FUNC VOID DIA_Bromor_business_Info()
{
    AI_Output (other, self ,"DIA_Bromor_business_15_01"); //Mam pewn¹ sprawê.
    AI_Output (self, other ,"DIA_Bromor_business_03_02"); //Wal.
    AI_Output (other, self ,"DIA_Bromor_business_15_03"); //Mam kolegê, który chce u ciebie pracowaæ.
    AI_Output (self, other ,"DIA_Bromor_business_03_04"); //Hahaha! Jako kto?
    AI_Output (other, self ,"DIA_Bromor_business_15_05"); //Jak to jako kto? A co to jest, warzywniak?
    AI_Output (other, self ,"DIA_Bromor_business_15_06"); //Oczywiœcie, ¿e jako dziwka.
    AI_Output (self, other ,"DIA_Bromor_business_03_07"); //Niech zgadnê, chodzi o tego obdarciucha Menelaosa?
    AI_Output (self, other ,"DIA_Bromor_business_03_08"); //Ostatnim razem chyba doœæ jasno siê wyrazi³em.
    AI_Output (other, self ,"DIA_Bromor_business_15_09"); //Bardzo mu na tym zale¿y. Poza tym popatrz na to z tej strony: zaczn¹ ciê odwiedzaæ klienci, powiedzmy, odmiennych preferencji.
    AI_Output (self, other ,"DIA_Bromor_business_03_10"); //Hmm, jak na to popatrzeæ to faktycznie niez³y pomys³.
    AI_Output (self, other ,"DIA_Bromor_business_03_11"); //Dobra! Przyjmê go.
    AI_Output (self, other ,"DIA_Bromor_business_03_12"); //Ale na razie tylko na okres próbny.
    AI_Output (other, self ,"DIA_Bromor_business_15_13"); //Przeka¿ê mu. Na pewno bêdzie zachwycony.
    B_LogEntry (TOPIC_Menel_Bitch,"Bromor zgodzi³ siê przyj¹æ Menelaosa.");
};
